...
def _outputs() -> dict:
    ...
    o["chapter_design_mapper"] = {
        "chapter_start": "A thirsty deer embodies longing for the living God.",
        "chapter_end": "The bowed-down living self waits for future praise.",
        "emotional_movement": "Thirst and memory descend into tears and overwhelming waters, then turn toward disciplined hope.",
        "divine_action": "God remains living, commands covenant love, gives song by night, and remains the object of future praise.",
        "physical_vocabulary": ["deer", "water", "thirst", "tears", "food", "waterfalls", "waves", "face"],
    ...
    o["theological_risk_agent"] = {"risks": [
        ...
        {"risk_id": "R2", "risk_description": "Call Psalm 42 a direct prophecy of Christ.", "why_it_matters": "The New Testament does not directly cite it as prophecy.", "avoidance_rule": "Name the link as thematic echo.", "evaluator_check": "Correspondence strength must be stated honestly."},
    ]}
    ...
    o["art_director"] = {
        ...
        "image_density": "one water system across thirst, tears, waterfalls, and promised streams",
        "emotional_color": "honest heaviness without sentimental closure",
        "opening_mode": "threshold phrase followed by sensory scene",
        "poem_tone": "common meter lament turning toward hope",
        "ending_resonance": "water promised without being possessed on demand",
        "avoid": ["clinical diagnosis", "forced prophecy", "instant relief"],
    }
    o["creative_divergence_agent"] = {
        "threshold_phrase_candidates": ["Hope speaks from inside the roar.", "Memory can deepen thirst.", "Thirst remembers where water was.", "The living self still waits.", "Sorrow does not cancel faith."],
        "introduction_opening_candidates": ["A deer lowers its head over an empty channel.", "The stones are smooth, but no water moves.", "Psalm 42 begins with a body that needs water."],
        "governing_image_candidates": ["a deer at an empty water channel", "tears eaten as food", "waterfalls becoming overwhelming waves"],
        ...
        "epigraph_candidates": ["Hope speaks from inside the roar.", "Memory can deepen thirst.", "Thirst remembers where water was."],
    }
    o["director"] = {
        ...
        "negative_constraints": ["do not diagnose", "do not promise immediate relief", "do not call the psalm direct prophecy"],
        "poem_plan": "common meter moving from dry channel to roaring deep and promised water",
    ...
    o["composer"] = {
        ...
        "introduction": "Hope speaks from inside the roar.\n\nA deer reaches an empty water channel and lowers its head. The stones are smooth, but no water moves between them. Dust clings to its mouth. Psalm 42 begins there.",
    ...
    o["evaluator"] = {
        ...
        "notes": "Psalm 42 preserves textual restraint, psychological plausibility, and a thematic Christological echo.",
    }
...
def test_psalm42_runs_full_v64_pipeline() -> None:
    ctx = run_engine(EngineContext(chapter_ref="Psalm 42"), MockAgentAdapter(_outputs()))
