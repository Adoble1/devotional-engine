...
def main() -> None:
    outputs = json.loads((Path(__file__).with_name("psalm24_mock_outputs.json")).read_text())
    config = EngineConfig(
    ...
    ctx = run_engine(EngineContext(chapter_ref="Psalm 24"), MockAgentAdapter(outputs), config)
